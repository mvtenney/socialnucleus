            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                ...
                @forelse ($campaign->tasks as $task)
                    <div class="flex flex-col justify-between p-4 bg-white rounded-md shadow-md hover:shadow-lg">
                        <div>
                            <h3 class="px-4 mt-4 mb-2 -ml-4 text-lg font-normal capitalize border-l-4 border-yellow-500"><a href="{{ route('view_task', $task) }}">{{ str_limit($task->title, 60) }}</a></h3>
                        ...
                        <div class="mt-6 mb-4 border-l-4 border-transparent">
                            <a href="{{ route('view_task', $task) }}" class="self-start px-4 py-2 my-4 text-gray-500 border rounded-md border-grey-500 hover:bg-yellow-500 hover:text-white">View Task</a>
                        </div>
                    </div>
                @empty
