        ...
        <div class="flex items-center justify-between">
            <div class="breadcrumbs">
                <h2 class="block text-sm font-semibold leading-none text-gray-800">
                    <a href="{{route('campaigns')}}">{{ __('Campaigns') }}</a> > <a href="{{route('view_campaign', $task->campaign)}}">{{$task->campaign->title}}</a> > <a href="{{route('tasks', $task->campaign)}}">{{ __('Tasks') }}</a> > {{ $task->title }}
                </h2>
            </div>
            <div class="controls">
                <a href="{{route('create_task', $task->campaign)}}" class="block yellow-btn">Create New Task</a>
            </div>
       </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-4">
                <div class="flex flex-col col-span-1 px-4 mb-6 md:col-span-2 lg:col-span-3">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{route('create_task', $task->campaign)}}" class="block green-btn">Save Changes</a>
                    </div>
